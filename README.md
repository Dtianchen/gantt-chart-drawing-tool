...
| 版本 | 日期 | 关键变更 |
|------|------|---------|
| **1.0.11** | 2026-04-29 | **JSON 导入/导出**备份恢复；**撤销/重做**（Ctrl+Z/Y，50步历史）；**任务完成百分比**（双层进度条+滑块编辑）；**实时搜索过滤**（Ctrl+F）；**全局快捷键**（Ctrl+N/Del/S/F）；提取共用 `getVisibleTasks` 过滤逻辑 |
| **1.0.10** | 2026-04-18 | **父任务进度条样式区分**：父任务（有子任务）使用斜纹半透明背景 + 加粗边框（2px）+ 更高高度（22px）；普通任务保持纯色实心 + 细边框（20px），视觉上清晰区分汇总任务与执行任务 |
